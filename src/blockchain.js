...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
           ...
           let error = await this.validateChain()
           if(!error){
               if (block.height < self.chain.push(block)){
                   resolve(block)
               } else {
    ...
    getStarsByWalletAddress (address) {
        ...
        return new Promise((resolve, reject) => {
            self.chain.forEach(function (block) {
                let jsonData = block.getBData()
                if(jsonData != null && jsonData.data.owner === address){
                    stars.push(jsonData.data);
                }
            });
            if (stars.length > 0){
                resolve(stars)
        ...
        return new Promise(async (resolve, reject) => {
            for (let index = 1; index < self.chain.length; index++) {
                ...
                if(currentBlock.previousBlockHash !== previousBlock.hash){
                    errorLog.push(`Block # ${index} invalid previous hash!!`)
                }
            }
